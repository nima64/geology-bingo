...
var gameS;
(function (gameS) {
    gameS[gameS["WAITING"] = 0] = "WAITING";
    gameS[gameS["PLAYING"] = 1] = "PLAYING";
})(gameS || (gameS = {}));
...
function simulateGuesses(players, maxNum) {
    let answer_stack = [];
...
function hasBingo(board, boardSize) {
    ...
    for (let i = 0; i < boardSize; i++) {
        ...
        let k = boardSize - 1 - i;
        //mat[i][i] != 1
        if (board[boardSize * i + i] != 1)
            full_lr_diag = false;
        //mat[k][k] != 1
        if (board[boardSize * k + k] != 1)
            full_rl_diag = false;
